    ...
    public static class Builder {
        ...
        public StateOfPerson build() throws HqdmException {
            if (stateOfPersonImpl.hasValue(AGGREGATED_INTO)
                    && stateOfPersonImpl.value(AGGREGATED_INTO).isEmpty()) {
                throw new HqdmException("Property Not Set: aggregated_into");
            }
            if (stateOfPersonImpl.hasValue(BEGINNING)
                    && stateOfPersonImpl.value(BEGINNING).isEmpty()) {
                throw new HqdmException("Property Not Set: beginning");
            }
            if (stateOfPersonImpl.hasValue(ENDING)
                    && stateOfPersonImpl.value(ENDING).isEmpty()) {
                throw new HqdmException("Property Not Set: ending");
            }
            if (stateOfPersonImpl.hasValue(MEMBER__OF)
                    && stateOfPersonImpl.value(MEMBER__OF).isEmpty()) {
                throw new HqdmException("Property Not Set: member__of");
            }
            if (stateOfPersonImpl.hasValue(MEMBER_OF)
                    && stateOfPersonImpl.value(MEMBER_OF).isEmpty()) {
                throw new HqdmException("Property Not Set: member_of");
            }
            if (stateOfPersonImpl.hasValue(PART__OF)
                    && stateOfPersonImpl.value(PART__OF).isEmpty()) {
                throw new HqdmException("Property Not Set: part__of");
            }
            if (!stateOfPersonImpl.hasValue(PART_OF_POSSIBLE_WORLD)) {
                throw new HqdmException("Property Not Set: part_of_possible_world");
            }
            if (stateOfPersonImpl.hasValue(TEMPORAL__PART_OF)
                    && stateOfPersonImpl.value(TEMPORAL__PART_OF).isEmpty()) {
                throw new HqdmException("Property Not Set: temporal__part_of");
            }
            if (stateOfPersonImpl.hasValue(TEMPORAL_PART_OF)
                    && stateOfPersonImpl.value(TEMPORAL_PART_OF).isEmpty()) {
                throw new HqdmException("Property Not Set: temporal_part_of");
            }
            return stateOfPersonImpl;
        }
    }
}
